...
def list_repositories(last):
	...
	try:
		for i in range(1, 11):
				print("Requesting https://api.github.com/search/repositories?q=language:java&per_page=100&page=" + str(i))
				req = urllib.request.Request("https://api.github.com/search/repositories?q=language:java&per_page=100&page=" + str(i))
				req.add_header('Accept', 'application/json')
				json_res = urllib.request.urlopen(req).read()
				json_data = json_res.decode('utf-8')
				...
				for repository in data["items"]:
					...
					if ((is_maven(contents_url) or is_gradle(contents_url))):
						print("Adding " + str(git_url))
						open("quick-repositories", "a").write(git_url + "\n")
